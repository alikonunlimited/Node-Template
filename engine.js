...
async function fetchLivePrice() {
  // SOURCE 1: Twelve Data (free, real-time)
  try {
    ...
    if (key) {
      ...
      if (r.ok) {
        ...
        if (p > 1800 && p < 7000) {
          lastKnownPrice = p;
          console.log(`[Price] Twelve Data → $${p}`);
          return { price: p, source: "Twelve Data" };
        }
        console.log(`[Price] Twelve Data bad response: ${JSON.stringify(d)}`);
      }
    }
  } catch (e) {
    console.log(`[Price] Twelve Data error: ${e.message}`);
  }
  ...
  try {
    ...
    console.log(`[Price] OANDA error: ${e.message}`);
  }
...
function decide(
  ...
  if (rsi < 40) {
    bull += 2;
    log.push(`RSI bull ${rsi.toFixed(0)}`);
  } else if (rsi > 60) {
    bear += 2;
    log.push(`RSI bear ${rsi.toFixed(0)}`);
  } else {
    log.push(`RSI neutral ${rsi.toFixed(0)}`);
  }
  if (stoch < 30) {
    bull += 2;
    log.push("Stoch OS");
  ...
  if (YOUR_PATTERNS.bestHoursUTC.includes(hourUTC)) {
    bull += 1;
    bear += 1;
  }
  ...
  return makeDecision(
    action,
    log.join(" · "),
    Math.min(Math.max(bull, bear) / 8, 1),
  );
}
